...
// Archives are not safe to edit or preview in-app; reveal them in the host file manager.
...
fn build_chat_file_link_plan(
    ...
    // Absolute/file-url links may use a home anchor; relative links keep a literal
    // leading "~" so a workspace entry named "~" remains addressable.
...
mod tests {
    ...
    fn archive_files_reveal_in_the_file_manager_instead_of_failing_closed() {
        let root = temp_workspace();
        for name in ["release.zip", "backup.tar", "export.7z", "logs.tgz"] {
    ...
    #[test]
    fn home_anchored_absolute_links_expand_but_relative_tilde_stays_literal() {
        ...
        let suffix = SystemTime::now()
            ...
            .expect("clock")
            .as_nanos();
        let home_dir = home.join(format!("liveagent-chat-file-links-home-{suffix}"));
        if fs::create_dir_all(&home_dir).is_err() {
            return;
        }
        fs::write(home_dir.join("notes.md"), "# home\n").expect("write home file");
        let root = temp_workspace();
        let planned = build_chat_file_link_plan(
            ...
            &format!(
                "~/{}/notes.md",
                home_dir.file_name().expect("home name").to_string_lossy()
            ),
            "absolute",
            None,
            None,
        ...
        fs::remove_dir_all(root).expect("remove temp workspace");
        fs::remove_dir_all(home_dir).expect("remove home dir");
    }
