...
// content identity (`checkpoint-<summaryId>`), so a colliding checkpoint is the
// same logical card seen from two sources — the history region and the
// (user-less) manual-compaction turn that streamed it. Renaming would render
// the card twice; the later copy is dropped instead.
