...
- **Safety policy:** the action is controller-backed in all execution modes and is blocked while the conversation is running, hydrating, or already compacting. Manual requests bypass only automatic threshold/cooldown checks; empty/disabled/in-flight guards remain authoritative.
- **Metadata/snapshot closure:** `contextUsageMetadata` now propagates through desktop assistant/token/checkpoint production, Gateway reducer/history/rows, context-ring derivation, and GUI runtime snapshot live/final/recovery paths. `runtime_state` is transport-only and is stripped before rendered rows while restoring status/retry state; checkpoint usage is preferred after compaction.
- **Task 6 re-audit:** broad shared transcript/status extraction remains deferred. `AssistantStatus` is already shared and both hosts have equivalent retry/checkpoint bodies; extracting `RetryDetailsBlock` or checkpoint cards would require a new shared prop/i18n/layout contract and risks replacing local transcript shells. See `upstream-adapted-2743-anthropic-context.md`.
