...
| 模型路由 | `task_type=search` 用于聚焦检索、代码导航、文档/日志调查和简单验证；`task_type=synthesis` 用于汇总、比较或整理已有发现。这两类显式任务在配置有效时使用全局 `subagentFastModel`。仅当默认关闭的 `customSettings.subagentProactiveDelegation` 开启时，schema 与运行时校验才允许 `task_type=routine`，用于边界清晰的日常实现、测试、重构和修复，并走同一 Fast 路由；关闭时必须保持旧 schema、错误文案与路由行为。字段省略、Fast 配置缺失或 Provider/模型失效时沿用当前父对话模型。Fast 请求若在首个 `text_delta`/`thinking_delta`/`toolcall_start` 提交前发生可切换的鉴权、配额、网络或上游错误，也只回退到该 Agent 批次快照的当前父模型；提交后不得整轮重跑，以免重复文本或工具副作用。架构设计、高风险/高影响决策和依赖大量父上下文的工作必须省略该字段。`task_type` 只属于本次 job，resume 不继承，需按新任务重新指定。 |
...
| 修改子代理任务路由或委派策略 | 同步检查 Agent schema、`parseSubagentBatch` 后端门控、Agent 工具 description、父模型 `buildToolsSuffix`、Fast/父模型选择、UI 协议恢复和相关测试。任何新增轻量任务类型都必须受显式设置约束；默认关闭时不得出现在 schema 中或被运行时接受。GUI 与 Gateway WebUI 的 `CustomSettings` 规范化、共享 Agents 设置页和两套 i18n 必须保持镜像一致。 |
